...
## preprocess 
data <- read.table("../data//SH000016.txt")
...
for(i in idx.2013:(n-1)){
  ## train a model
  model <- trainESN(data = data$V6, trainLen = i, inSize = inSize, trainLog = F, alpha = 0.9, leakingRate = 0.1)
...
save(pred, file = "../result/slideWinPred_SZ50.RData")
...
chart.CumReturns(profit.xts,geometric = F, ylim = c(-0.2, 1), type = "l", 
                 col = "blue", main = "Cumulative Return Plots of ESN and SZ50"
)

lines(cumsum(targetY), type = "l", col = "red")
legend("topleft",bty = "n", ncol = 2, cex = 0.5,  legend = c("Standard ESN", "SZ50"), col=c("blue","red"), lwd = 2)
...
chart.CumReturns(profit.xts.2013,geometric = F, ylim = c(-0.2, 0.2), type = "l", 
                 col = "blue", main = "2013 Cumulative Return Plots of ESN and SZ50"
)
lines(cumsum(checkData(targetY.xts["2013"], method = "vector")), type = "l", col = "red")
legend("topright",bty = "n", ncol = 2, cex = 1,  legend = c("Standard ESN", "SZ50"), col=c("blue","red"), lwd = 2)


profit.xts.2014 <- profit.xts["2014"]
chart.CumReturns(profit.xts.2014,geometric = F, ylim = c(-0.2, 0.6), type = "l", 
                 col = "blue", main = "2014 Cumulative Return Plots of ESN and SZ50"
)
...
chart.CumReturns(profit.xts.2015,geometric = F, ylim = c(-0.2, 0.6), type = "l", 
                 col = "blue", main = "2015 Cumulative Return Plots of ESN and SZ50"
)
